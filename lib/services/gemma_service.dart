...
class GemmaService {
  ...
  Stream<String> generateFeatureResponseStream({
    // celle ci use chat pas single ssssion car cette dernièe non focntionnelle non stable pour le moment
  ...
  Stream<String> generateFeatureResponseStreamSingle({
    required String prompt,
    required Uint8List imageBytes,
  }) {
  ...
  // NOUVEAU : Méthode pour gérer l'erreur de configuration hors-ligne
  void handleOfflineSetupError() {
    errorMessage.value = "An internet connection is required for the initial setup. Please connect and retry.";
    _updateState(GemmaState.error);
  }

  // NOUVEAU : Méthode pour gérer l'échec de la configuration du token
  void handleTokenConfigurationFailed() {
    errorMessage.value = "A Hugging Face token is required to continue. Please retry.";
    _updateState(GemmaState.error);
  }
}
